...
/*
 * This Adapter is used in GroupChat Window
 */
public class GroupChatAdapter extends CursorAdapter {
	private static ImageLoader imageLoader;
	private static String userJid;
	private static Context mContext; 
	public GroupChatAdapter(Context context, Cursor c,String userJid) {
	...
	private static class ViewHolder implements OnClickListener{
		...
		private TextView extraMessageTV;
		
		

		private ViewHolder(ViewGroup parent) {
			Context context = parent.getContext();
			chatRowParentView =(ViewGroup)LayoutInflater.from(context).inflate(R.layout.row_group_chat_message, parent, false);
			extraMessageTV = (TextView)chatRowParentView.findViewById(R.id.tv_extra_message);
			...
			senderNameTV = (TextView)receivedChatView.findViewById(R.id.tv_contact_name);
		...
		public void bind(Cursor cursor){
			...
			String sender = cursor.getString(cursor.getColumnIndex(AppDatabaseConstants.USER_CHAT_ID));
			//Time logic
			...
			if(coversationType.equalsIgnoreCase("1")){
				extraMessageTV.setVisibility(View.GONE);
				...
				String senderNumber = cursor.getString(cursor.getColumnIndex(AppDatabaseConstants.USER_CHAT_ID));
				if(!senderNumber.matches(".*[a-zA-Z]+.*")){
					senderNumber  = ApplicationDAO.getInstance(mContext).getUserNameFromCommUser(null, senderNumber);
				}
				senderNameTV.setText(senderNumber);
				if(messageType.equalsIgnoreCase(XmppConstants.TYPE_TEXT)){
					extraMessageTV.setVisibility(View.GONE);
				...
				}else if (messageType.equalsIgnoreCase(XmppConstants.TYPE_EDIT_GROUP)) {
					extraMessageTV.setText(cursor.getString(cursor.getColumnIndex(AppDatabaseConstants.COLUMN_MESSAGE)));
					extraMessageTV.setVisibility(View.VISIBLE);
					receivedChatView.setVisibility(View.GONE);
				}
